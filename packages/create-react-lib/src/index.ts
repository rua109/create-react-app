...
const questions = [
  ...
  },
  {
    ...
    choices: [
      { title: "not using react", value: "none" },
  ...
  {
    type: (prev: string) => (prev !== "none" ? "confirm" : null),
    name: "storybook",
  ...
  },
  {
    type: (prev: any, values: any) =>
      values.reactVersion != "none" ? "confirm" : null,
    name: "tailwind",
...
const CONFIG_REACT = path.resolve(__dirname, "../templates/config-react");
...
const CONFIG_SCSS = path.resolve(__dirname, "../templates/config-scss");
...
(async () => {
  ...
  if (esjOptions.usesReact) {
    copyDirApplyingEjsTransforms(CONFIG_REACT, folder, esjOptions);
    if (esjOptions.usesTailwind) {
      copyDirApplyingEjsTransforms(CONFIG_TAILWIND, folder, esjOptions);
    } else {
      copyDirApplyingEjsTransforms(CONFIG_SCSS, folder, esjOptions);
    }
  }
  if (esjOptions.transpiler === "babel") {
